...
func _build_level() -> void:
	...
	for brick_x: int in [16, 17, 18, 59, 60, 61, 106, 107, 108]:
	...
	add_block(Vector2(62 * 16 + 8, 7 * 16 + 8), GearBlock.Type.QUESTION, GearBlock.Content.MUSHROOM)
	add_collectible_arc(Vector2(420, 118), 6)
	add_collectible_arc(Vector2(785, 105), 6)
	add_collectible_arc(Vector2(1510, 108), 7)
	add_enemy(Vector2(270, 174), PooledEnemy.Kind.BOUNCECAP, 58, false)
	...
	add_cactus(Vector2(720, 192), false)
	add_cactus(Vector2(1630, 192), true, 2.0)
	...
	add_checkpoint(Vector2(1215, 188))
