...
func _run() -> void:
	...
	manager.current_level_id = "world_2_3"
	...
	save.save_checkpoint("world_2_3", SAVED_CHECKPOINT, 6, 3450, 27, 2)
	...
	await process_frame
	await physics_frame
	_check(first_level.level_id == "world_2_3", "saved campaign stage selects World 2-3")
	...
	_check(reloaded.level_id == "world_2_3", "reload keeps the saved stage")
