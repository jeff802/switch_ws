...
const OLD_CAMPAIGN_PATH := "/private/tmp/forest_gear_v2_save_probe.json"
...
func _run() -> void:
	...
	writer.completed_stages.assign(["stage_01", "stage_02"])
	writer.save_checkpoint("stage_07", Vector2(512, 144), 6, 3450, 27, 2, 2)
	...
	_check(raw is Dictionary and int(raw.get("version", 0)) == 5, "save file carries schema version 5")
	writer.queue_free()
	await process_frame
	...
	_check(reader.carried_reserve_bloom_count == 2, "two reserved energy blooms reload")
	_check(reader.get_checkpoint("stage_07", Vector2.ZERO) == Vector2(512, 144), "checkpoint reloads")
	...
	_check(legacy_reader.carried_reserve_bloom_count == 0, "legacy save receives empty reserve slots")
	_check(legacy_reader.is_level_unlocked("cave"), "legacy unlocks are preserved")
	legacy_reader.queue_free()
	await process_frame

	var old_campaign_file := FileAccess.open(OLD_CAMPAIGN_PATH, FileAccess.WRITE)
	old_campaign_file.store_string(JSON.stringify({
		"version": 4,
		"campaign_stage": 31,
		"carried_reserve_bloom": true,
		"keyboard_bindings": {},
	}))
	old_campaign_file.close()
	var old_campaign_reader = save_script.new()
	old_campaign_reader.save_path = OLD_CAMPAIGN_PATH
	root.add_child(old_campaign_reader)
	_check(old_campaign_reader.campaign_stage == 19, "older oversized campaign saves clamp to the final stage of the 20-stage campaign")
	_check(old_campaign_reader.carried_reserve_bloom_count == 1, "single-slot version 4 reserve migrates into the first new slot")
	old_campaign_reader.queue_free()
	await process_frame
...
func _cleanup() -> void:
	for path: String in [PROBE_PATH, LEGACY_PATH, OLD_CAMPAIGN_PATH]:
